...
// Custom max_length function with E axis support for bioprinter homing
inline float max_length(const AxisEnum axis) {
  if (axis == E_AXIS) return E_MAX_LENGTH;
  static const XYZval<float> max_length_P DEFS_PROGMEM = NUM_AXIS_ARRAY(X_MAX_LENGTH, Y_MAX_LENGTH, Z_MAX_LENGTH, I_MAX_LENGTH, J_MAX_LENGTH, K_MAX_LENGTH, U_MAX_LENGTH, V_MAX_LENGTH, W_MAX_LENGTH);
  return pgm_read_any(&max_length_P[axis]);
}
